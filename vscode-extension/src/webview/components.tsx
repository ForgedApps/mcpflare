...
export const ShieldLogo: React.FC<IconProps> = ({ size = 48 }) => (
  <svg width={size} height={size} viewBox="0 0 128 128" fill="none">
    ...
    {/* Subtle shadow */}
    <path
      d="M64 12L112 26V54C112 84 90 106 64 117C38 106 16 84 16 54V26L64 12Z"
      fill="#0f172a"
      opacity="0.15"
    ...
    {/* Main shield */}
    <path
      d="M64 10L114 25V54C114 85 91 108 64 119C37 108 14 85 14 54V25L64 10Z"
      fill="url(#shieldMainLogo)"
    />
    {/* Left highlight */}
    <path
      d="M64 10L14 25V54C14 85 37 108 64 119L64 10Z"
      fill="white"
      opacity="0.15"
    />
    {/* Right shadow */}
    <path
      d="M64 10L114 25V54C114 85 91 108 64 119L64 10Z"
...
export const ShieldOffIcon: React.FC<IconProps> = ({
  size = 20,
  className,
}) => (
  <svg
...
const getTokenStatus = (
  tokens: number,
  contextWindowSize: number,
): { label: string; color: string; severity: 'low' | 'high' | 'critical' } => {
  const percentage = (tokens / contextWindowSize) * 100
  if (percentage < 2.5)
    return { label: 'Excellent', color: '#22c55e', severity: 'low' }
  if (percentage < 5)
    return { label: 'Good', color: '#84cc16', severity: 'low' }
  if (percentage < 10)
    return { label: 'High', color: '#f97316', severity: 'high' }
...
export const TokenSavingsBadge: React.FC<TokenSavingsBadgeProps> = ({
  ...
  // Calculate totals
  const unguardedTokens = unguardedMcps.reduce(
    (sum, mcp) => sum + mcp.tokens,
    0,
  )
  const guardedTokensOriginal = guardedMcps.reduce(
    (sum, mcp) => sum + mcp.tokens,
    0,
  )
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {isEditingContext ? (
            ...
            <span
              ...
              title={
                onContextWindowChange
                  ? 'Click to change context window size'
                  : 'Context window size'
              }
              style={{
                fontSize: '11px',
            ...
            >
              {contextPercentage.toFixed(1)}% of{' '}
              {formatCompact(effectiveContextWindow)}
            </span>
          )}
        </div>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', height: '100%', width: '100%' }}>
          {mcpsWithTokens.map((mcp, index) => {
            ...
            const segmentWidth =
              totalTokensForBar > 0 ? (mcp.tokens / totalTokensForBar) * 100 : 0
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderLeft:
                    index === 0
                      ? `2px solid ${mcp.isGuarded ? GUARDED_GREEN : UNGUARDED_YELLOW}`
                      : 'none',
                  borderRight: isLast
                    ? `2px solid ${mcp.isGuarded ? GUARDED_GREEN : UNGUARDED_YELLOW}`
                    : '1px solid rgba(0,0,0,0.3)',
                  borderRadius:
                    index === 0 ? '4px 0 0 4px' : isLast ? '0 4px 4px 0' : '0',
              ...
              >
                ...
                {segmentWidth > 12 && (
                  ...
                  >
                    ...
                    {!mcp.isGuarded && (
                      <ShieldOffIcon size={12} className={undefined} />
                    )}
                    {mcp.name}
                  </span>
                )}
        ...
        {hoveredMCP ? (
          ...
          >
            {(() => {
              ...
              const mcpBarPercent =
                totalTokensForBar > 0
                  ? ((mcp.tokens / totalTokensForBar) * 100).toFixed(0)
                  : '0'
              const mcpContextPercent = (
                (mcp.tokens / effectiveContextWindow) *
                100
              ).toFixed(2)
              ...
              if (mcp.isGuarded) {
                return (
                  <>
                    ...
                    />
                    <ShieldIcon size={12} className={undefined} />
                    <span style={{ fontWeight: 600, color: GUARDED_GREEN }}>
                      {mcp.name}
                    </span>
                    ...
                    <span style={{ color: 'var(--text-muted)' }}>·</span>
                    <span style={{ color: GUARDED_GREEN, fontWeight: 500 }}>
                      guarded
                    </span>
                  </>
                )
              ...
              return (
                <>
                  ...
                  <span>{mcp.toolCount} tools</span>
                  <span style={{ color: 'var(--text-muted)' }}>·</span>
                  <span style={{ color: mcp.color, fontWeight: 600 }}>
                    {mcpBarPercent}%
                  </span>
                  <span style={{ color: 'var(--text-muted)' }}>·</span>
                  <span style={{ color: UNGUARDED_YELLOW, fontWeight: 500 }}>
                    unguarded
                  </span>
                </>
              )
        ...
        unguardedTokens > MCPGUARD_BASELINE_TOKENS && (
          ...
          >
            ...
            <span style={{ color: 'var(--text-secondary)' }}>
              Guard {unguardedMcps.length} MCP
          ...
          <div>
            <span style={{ fontWeight: 600, color: status.color }}>
              {status.severity === 'critical'
                ? 'High impact on LLM performance. '
                : 'Notable context consumption. '}
            </span>
            {status.severity === 'critical'
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {server.tokenMetrics && !server.tokenMetrics.packageName && (
              ...
              >
                {server.tokenMetrics.toolCount} tools ·{' '}
            ...
            {!server.tokenMetrics &&
              ...
              server.assessmentError.type !== 'auth_failed' && (
                <span
                  ...
                  title={server.assessmentError.message}
                >
                  {server.assessmentError.type === 'connection_failed'
                    ? 'connection failed'
                    : server.assessmentError.type === 'timeout'
                      ? 'timeout'
                      : 'error'}
                </span>
              )}
                ...
                <div style={{ flex: 1 }}>
                  <div
                    style={{
                      ...
                      fontSize: '12px',
                      marginBottom: '4px',
                      color: server.headers ? 'var(--warning)' : 'var(--error)',
                    }}
                  >
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    {server.tokenMetrics.schemaChars.toLocaleString()} chars
                  </div>
                </div>
                <div>
                  <div
                ...
                >
                  Assessed{' '}
                  {new Date(
                    server.tokenMetrics.assessedAt,
                  ).toLocaleDateString()}
                </div>
              </div>
            </div>
...
export const Notification: React.FC<NotificationProps> = ({
  ...
  >
    ...
    <span
      onClick={onDismiss}
      style={{ cursor: 'pointer', opacity: 0.5, fontSize: '16px' }}
    >
      ×
    </span>
  </div>
    ...
    >
      ...
      <div>
        ...
        >
          ...
          reducing context window token usage by up to 98%
        </p>
      </div>
    </div>
...
export const EmptyState: React.FC<{ onImport: () => void }> = ({
  onImport,
}) => (
  <div
    style={{
...
Just report PASS or FAIL for each test.`,
      ...
      case 'network':
        return {
          ...
          prompt: `Test network isolation for "${mcpName}". Run using call_mcp with mcp_name "${mcpName}":
...
Just report PASS or FAIL.`,
      ...
      case 'codeInjection':
        return {
          ...
          description:
            'Verify that eval() and Function constructor are blocked.',
          prompt: `Test code injection prevention for "${mcpName}". Run using call_mcp with mcp_name "${mcpName}":

\`\`\`typescript
let pass = 0;
try { eval('1'); } catch { pass++; console.log('PASS: eval blocked'); }
try { new Function('1'); } catch { pass++; console.log('PASS: Function blocked'); }
console.log(pass === 2 ? 'RESULT: Guarded' : 'RESULT: Vulnerable');
\`\`\`
...
Just report the result.`,
      ...
      case 'filesystem':
        return {
          ...
          prompt: `Test filesystem isolation for "${mcpName}". Run using call_mcp with mcp_name "${mcpName}":
        ...
        >
          ...
          <div style={{ padding: '20px' }}>
            ...
            >
              ...
              >
                <li>
                  <strong>Prompt Injection</strong> — Malicious content in MCP
                  responses could trick the AI into harmful actions
                </li>
                <li>
                  <strong>Data Exfiltration</strong> — Compromised code could
                  send sensitive data to external servers
                </li>
                <li>
                  <strong>Unauthorized Access</strong> — Code could attempt to
                  access files or resources beyond the MCP's scope
                </li>
              </ul>
            </div>
            ...
            >
              ...
              >
                <li>
                  <strong>Network Isolation</strong> — All external network
                  requests are blocked
                </li>
                <li>
                  <strong>Sandboxed Execution</strong> — Code runs in an
                  isolated Cloudflare Worker
                </li>
                <li>
                  <strong>Code Validation</strong> — Dangerous patterns like
                  eval() are blocked before execution
                </li>
                <li>
                  <strong>Legitimate Calls Work</strong> — Normal MCP tool calls
                  function correctly
                </li>
              </ul>
            </div>
            ...
            >
              Enable guarding for this MCP, then run security tests to verify
        ...
        <div style={{ padding: '20px', overflow: 'auto', flex: 1 }}>
          ...
          {testResult && (
            <>
              ...
              >
                <strong>Need more details?</strong> Click "View Logs" to see
                full request/response data in the Output panel.
              </div>
            </>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasData && <ChevronDownIcon size={14} className={undefined} />}
      </div>
...
interface TestTypeSelectorProps {
  selectedType:
    ...
    | 'legitimate'
  onSelectType: (
    type: 'quick' | 'network' | 'codeInjection' | 'filesystem' | 'legitimate',
  ) => void
}
...
export const TestTypeSelector: React.FC<TestTypeSelectorProps> = ({
  ...
  const testTypes = [
    ...
    {
      id: 'legitimate' as const,
      name: 'Legitimate Call',
      description: 'Test normal tool usage',
      icon: <CheckIcon size={14} />,
    },
    {
      id: 'network' as const,
      name: 'Network Isolation',
      description: 'Test fetch() blocking',
      icon: <NetworkIcon size={14} />,
    },
    {
      id: 'codeInjection' as const,
      name: 'Code Injection',
    ...
    },
    {
      ...
      name: 'Filesystem',
      description: 'Test fs access blocking',
        ...
        >
          <div
            style={{
              ...
              borderRadius: 'var(--radius-sm)',
              background:
                selectedType === test.id ? '#22c55e' : 'var(--bg-hover)',
              color:
                selectedType === test.id ? 'white' : 'var(--text-secondary)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
            }}
...
export const TestingTab: React.FC<TestingTabProps> = ({
  ...
  const [selectedTestType, setSelectedTestType] = useState<
    'quick' | 'network' | 'codeInjection' | 'filesystem' | 'legitimate'
  >('quick')
  ...
  const securityFeatures = [
    {
      ...
      description:
        'Workers cannot make arbitrary network requests. All fetch() and HTTP calls are blocked.',
      color: '#3b82f6',
    },
    {
      ...
      description:
        'Dangerous patterns like eval(), Function constructor, and require() are blocked.',
      color: '#22c55e',
    },
    {
    ...
    <div style={{ padding: '16px' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {servers.length > 0 ? (
              ...
              >
                No MCPs available. Import MCPs from your IDE configuration
                first.
              </p>
            )}
          </div>
        ...
        {selectedMCP && (
          <div
            style={{
              ...
              padding: '12px 16px',
              borderRadius: 'var(--radius-md)',
              background: selectedMCPIsGuarded
                ? 'rgba(34, 197, 94, 0.1)'
                : 'rgba(255, 215, 0, 0.1)',
        ...
        {selectedMCP && (
          <div style={{ marginBottom: '16px' }} className="animate-fade-in">
            ...
            <TestTypeSelector
              ...
              onSelectType={setSelectedTestType}
            />
          </div>
        )}
      ...
      >
        ...
        >
          ...
          tool, which runs code in a secure, isolated Cloudflare Worker
        ...
        >
          <li>
            <strong>Legitimate tool calls</strong> work correctly through the
            isolation layer
          </li>
          <li>
            <strong>Network requests</strong> to external servers are blocked
            (prevents data exfiltration)
          </li>
          <li>
            <strong>Dangerous code patterns</strong> like eval() are blocked
            (prevents code injection)
          </li>
          <li>
            <strong>Filesystem access</strong> is unavailable (prevents
