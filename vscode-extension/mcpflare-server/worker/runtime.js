...
export class FetchProxy extends WorkerEntrypoint {
    async fetch(request) {
        const allowedHostsHeader = request.headers.get('X-MCPflare-Allowed-Hosts') || '';
        const allowLocalhostHeader = request.headers.get('X-MCPflare-Allow-Localhost') || 'false';
        const allowedHosts = allowedHostsHeader
            ? allowedHostsHeader
                .split(',')
                .map((h) => h.trim())
                .filter((h) => h)
    ...
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            const dynamicWorker = env.LOADER.get(workerId, async () => {
                ...
                const fetchProxy = needsFetchProxy &&
