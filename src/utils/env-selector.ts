...
export async function selectEnvVarsInteractively(
  ...
  console.log('\n💡 Options:')
  console.log(
    `  - Enter a number (1-${envVarKeys.length}) to select an env var`,
  )
  ...
  while (true) {
    ...
    const input = await new Promise<string>((resolve) => {
      rl.question(
        'Select env var by number (or "done"/"skip"/"manual"): ',
        resolve,
      )
    })
    ...
    if (Number.isNaN(num) || num < 1 || num > envVarKeys.length) {
      console.log(
        `❌ Invalid number. Please enter a number between 1 and ${envVarKeys.length}, or "done"/"skip"/"manual".\n`,
      )
      continue
    }
