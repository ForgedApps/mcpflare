...
<Tabs items={['Attack', 'Protection']}>
  ...
  <Tab value="Protection">
    - ✅ Default: `globalOutbound: null` - `fetch()` does not exist, calls throw errors
    - ✅ Three-layer network enforcement when enabled:
      1. Runtime isolation: `globalOutbound: null` (no native fetch)
      2. Module-level wrapper: Injected fetch adds allowlist headers
      3. FetchProxy Service Binding: Enforces host allowlist in parent Worker
...
<Tabs items={['Attack', 'Protection']}>
  ...
  <Tab value="Protection">
    - ✅ Default: no network - `fetch()` does not exist, throws errors
    - ✅ Three-layer enforcement when network enabled: FetchProxy Service Binding validates all requests
