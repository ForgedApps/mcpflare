...
Click the **shield icon** 🛡️ in the activity bar (left sidebar) to open MCPflare.
...
<Callout type="info" title="What this controls">
  These settings control **network access from code executed inside MCPflare’s Worker isolate**. When enabled, MCPflare allows `fetch()` in the isolate but enforces the host allowlist (and optional localhost).
</Callout>
...
3. Check the Output panel (View → Output → MCPflare) for errors
...
- [Documentation](https://github.com/forgedapps/mcpflare)
- [Report Issues](https://github.com/forgedapps/mcpflare/issues)
