...
### Fixed
- Prevented VS Code extension packaged server startup failures caused by ESM dynamic `require(...)` runtime errors (for example, `Dynamic require of "fs" is not supported`).

### Changed
- Added a packaged-server startup verification gate (`vscode-extension/scripts/verify-packaged-server.js`) and wired it into extension prepublish plus release workflow checks before marketplace publishing.
